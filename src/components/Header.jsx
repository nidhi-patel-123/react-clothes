      ...
      <div className='flexBetween py-3'>
        ...
        <Link to={'/'} className='flex flex-1 '>
          <div className='bold-32'>
            CLOTHE<span className='text-teal-500'>S</span>
          </div>
        </Link>
        {/* navbar--- */}
        <div className='flex-1'>
          <Navbar containerStyles={`${menuOpened ? "flex item-start flex-col gap-y-8 fixed top-16 right-6 p-5 bg-white rounded-xl shadow-md w-52 ring-1 ring-slate-900/5 z-50" : "hidden xl:flex gap-x-5 xl:gap-x-10 medium-15 ring-1 ring-slate-900/5 rounded-full p-1 "}`} />
        </div>
        {/* Buttons right side */}
        <div className='flex-1 flex items-center justify-end gap-x-2 xs:gap-x-8'>
          ...
          <Link to={'/cart'} className='flex relative'>
            ...
            <span className='bg-teal-500 text-white text-[12px] font-semibold absolute left-1.5 -top-3.5 flexCenter w-4 h-4 rounded-full shadow-md '>{getCartCount()}</span>
          </Link>
          {/* user profile */}

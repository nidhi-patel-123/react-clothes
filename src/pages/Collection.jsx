...
function Collection() {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1)//reset to the first page when filter change
        ...
        <div className='bg-primary p-4 rounded-l-xl'>
          ...
          <div className='flexCenter flex-wrap mt-14 gap-4 mb-10'>
            <button disabled={currentPage === 1} onClick={() => setCurrentPage((prev) => prev - 1)}
              className={`${currentPage === 1 &&  "opacity-50 cursor-not-allowed text-white"} btn-secondary !py-1 !px-3`}
              >Previous</button>
              {Array.from({length: totalPages},(_, index)=>(
                <button
                key={index + 1} onClick={()=>setCurrentPage(index + 1)}
                className={`${currentPage === index + 1 && "!bg-tertiary !text-white rounded-full"}btn-light !py-1 !px-3 `}
                >
                  {index + 1}
                </button>
              ))}
               <button disabled={currentPage === totalPages} onClick={() => setCurrentPage((prev) => prev + 1)}
              className={`${currentPage === totalPages &&  "opacity-50 cursor-not-allowed text-white"} btn-secondary !py-1 !px-3`}
              >Next</button>
          </div>
        </div>
      </div>
